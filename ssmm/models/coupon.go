...
type Coupon struct {
	...
	Recursion   int8      `orm:"column(recursion)"`
	Totaltimes  int       `orm:"column(totaltimes)"`
	Usedtimes   int       `orm:"column(usedtimes)"`
	Isactive    int8      `orm:"column(isactive)"`
	Effecttime  time.Time `orm:"column(effecttime);type(datetime)"`
	Expiretime  time.Time `orm:"column(expiretime);type(datetime)"`
	Createtime  time.Time `orm:"column(createtime);type(timestamp)"`
}
